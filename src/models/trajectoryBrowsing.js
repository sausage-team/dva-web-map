...
export default {
  ...
  effects: {
    *getPoliceList({ payload:num }, { call, put, select }) {
      ...
      let PoliceListData = yield call(getHotMap, 'sm/3d/queryCarList/'+num+'', {});
          ...
          if (data[treeData[j].key]) {
            for (let i in data[treeData[j].key]) {
              let obj = {
                ...
                key: '警车' + data[treeData[j].key][i].username,
    ...
    *getPoliceLocation({ payload:arr }, { call, put, select }) {
      ...
      let PoliceListData = yield call(getHotMap, 'sm/3d/queryCarLocation/'+outArr.toString()+'', {});
      if(PoliceListData.code == 0){
        let data = PoliceListData.data;
        let obj = {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/TrajectoryBrowsing') {
